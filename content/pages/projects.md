...
Python-библиотека предоставляет функционал по авторизации, логированию,
...
сенсоров, записи журналов, общее состояние, управлять
...
журнала, изменение инвентарных данных). Основная проблема - работа с
медленными железками и нестабильной сетью. Реализовано в виде монолита,
где часть логики по сбору и обработке информации от оборудования
...
- Разработка бизнес логики на бэкенде
- Разработка корпоративных инструментов логирования и генерации документации
...
составления перечня компонентов оборудования (серверов, СХД и прочего), а
...
- Разработка оптимизатора (на основе [`HiGHS`](https://highs.dev/) и библиотеки [`PuLP`](https://github.com/coin-or/pulp))
- Разработка бэкенд части с бизнес логикой и задачами оптимизации, запускаемыми через `Celery`
...
- Сбор и формализация требований
- Наставничество стажеров (выступал в роли техлида команды)
...
пользователя с возможностью записи к врачу, личный кабинет врача с
возможностью просматривать расписание и анкеты пациентов, модуль тестирования
...
Desktop приложение раскроя слитка оптимизирует последовательность
...
- Разработка оптимизатора для модифицированной задачи
